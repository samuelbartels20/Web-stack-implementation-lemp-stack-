...
- <span style="color:red">location ~ /\.ht</span> — The last location block deals with <span style="color:red">location ~ /\.ht</span>, which Nginx does not process. By adding the deny all directive, if any .htaccess files happen to find their way into the document root ,they will not be served to visitors.